...
class CO():
    def __init__(self, f):
        ...
        for idx, b in enumerate(boundaries):
            ...
            '''
            p.append(init_factor*Wave(array([1.]+[0.]*(b-1)).astype('float32')))
        self.p = tf.Variable(initial_value=tf.reshape(tf.ragged.stack(p), -1), dtype=tf.float32, trainable=True)
    def maximize(self, n_steps = 200, lr = 1e-1, n_samples = 1):
        answers = []
        ...
        while early_stopping < n_steps:
            ...
            if self.best_answer[-1]<answers[0][-1]:
                ...
                self.p.assign(Wave(p_nu).astype('float32')*init_factor)
