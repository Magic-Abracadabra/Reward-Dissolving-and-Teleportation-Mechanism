...
def write(*x):
    with open('D:\\毕业论文\\ε-贪心策略.txt', 'a') as file:
        for item in x:
            file.write(str(item)+' ')
        file.write('\n')
        file.close()
...
def exploration_policy(Qs, epsilon):
    choice = tf.random.categorical(logits=tf.math.log([[epsilon, 1-epsilon]]), num_samples=1)
    if choice==0:
        return tf.stack(list(map(lambda Q: tf.reshape(tf.random.categorical(logits=tf.stack(Q)[None]*0., num_samples=1), ()), Qs)))
    else:
        return tf.stack(list(map(lambda Q: Q.argmax(), Qs)))
...
class CO():
    def __init__(self, f):
        ...
        for idx, b in enumerate(boundaries):
            '''
            if idx==init:
                self.Qs.append(init_factor*array([0.]*(b-1)+[1.]))
            else:
                self.Qs.append(init_factor*array([1.]+[0.]*(b-1)))
            '''
            self.Qs.append(init_factor*Wave(array([1.]+[0.]*(b-1))))
    def maximize(self, tolerance = 200, lr0 = 1e-1):
        lr = lr0
        old_best_value = 0.
        count_tolerance = 0
        ...
        while count_tolerance<tolerance:
            ...
            epsilon = 1.-0.95*count_tolerance/tolerance
            A = exploration_policy(self.Qs, epsilon).numpy()
            r = self.f(A, C)
            print('current try',r)
            bests.append((A, r))
